# simplified-vpn-interface Specification
...
### Requirement: Compact appearance defaults

Fresh application settings SHALL use dark mode, a muted teal seed color (`#4F9D92`), the Tonal Spot color scheme, pure-black mode disabled, and an enabled 80% text-size override. Existing explicitly saved appearance settings MUST remain unchanged on upgrade. Settings SHALL retain light, dark, and system-following modes, custom colors, scheme selection, pure black, and adjustable text size. Disabling the text-size override SHALL restore system-derived scaling within the supported 80–140% range. Resetting the color palette SHALL restore the teal seed and Tonal Spot scheme without resetting text size or theme mode.

#### Scenario: Fresh settings and existing preferences

- **WHEN** the application starts without saved appearance settings
- **THEN** it uses dark teal Tonal Spot styling and 80% text size
- **AND** loading explicitly saved colors, scheme, mode, or text-size choices preserves those choices

#### Scenario: Adjust or reset appearance

- **WHEN** the user resets the color palette in Settings
- **THEN** the teal seed and Tonal Spot scheme are restored without changing their text-size or theme-mode choice
- **AND** the text-size override can be adjusted or disabled independently
